...
**True always-on wake word — listening after the app is force-stopped, killed, or
swiped away — is not implemented, and this is documented rather than faked.**
What *is* implemented and does work: turning on "Wake word" in Settings starts a real
foreground service (`NovaVoiceService`) that keeps listening for "Nova" after you
leave the app (Home button, app switcher, screen off), with an ongoing notification
you can stop from at any time — this is Android's actual sanctioned way for an app to
use the microphone while not in the foreground, not a workaround. What it cannot do,
because no ordinary app is allowed to on stock Android: keep listening after you
force-stop it from App Info, after some OEM launchers kill background services on
"clear all", or fast enough after the OS kills the process outright under memory
pressure. Real "app fully closed, still always listening" hotword detection needs
either a licensed low-power DSP SDK (e.g. Picovoice Porcupine) or OEM-level privileges
that regular apps don't have. `voice/WakeWordManager.kt` and
`voice/BackgroundListeningEngine.kt` document and implement the legitimate version in
full: while the service is running it loops listen-for-"Nova" → speak "Igen?"/"Yes?" →
listen for the command → route it (same `CommandRouter`/`AIProvider` path as in-app
chat) → speak the reply → back to listening, all inside the foreground service so it
doesn't depend on any Activity/ViewModel being alive. Manual activation (tapping the
mic in Home/Chat) always works regardless of this setting. Swapping in a dedicated
hotword SDK later would only mean changing `BackgroundListeningEngine`/
`WakeWordManager` — no other layer depends on how wake-word detection is implemented.
...
- Background listening requests `RECORD_AUDIO` and, on Android 13+, `POST_NOTIFICATIONS`
  at runtime; if either is denied, the "Wake word" setting turns itself back off with
  an on-screen explanation rather than silently failing.
- If the OS kills `NovaVoiceService`'s process under memory pressure, `START_STICKY`
  tells Android to try restarting it, but any command it was mid-way through handling
  is lost — this is disclosed, not hidden behind a "seamless" claim.
